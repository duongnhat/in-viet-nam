...
@section('description', 'Trang quản lý hình ảnh.')
@section('content')
    <div class="container-fluid">
        <h3 class="card-header text-center bg-secondary text-light text-uppercase">Quản lý hình ảnh của {{$product->name}}</h3>
        @if($list->count() == 0)
                    ...
                    @foreach($list as $i => $image)
                        <tr>
                            ...
                            <td class="text-center">
                                <a onclick="return confirm('Bạn có chắc chắn muốn xóa?');" href="/admin/product/delete-image/{{$image->id}}">
        ...
        @endif
        <a type="button" href="/admin/product/quan-ly-san-pham" class="btn btn-danger">Trở lại</a>
    </div>
@endsection
